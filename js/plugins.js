...
function prettyMetric(x, metric) {
    if (isNumber(x)) {
        ...
        return (HUCMeta[metric].style.prefix ? HUCMeta[activeMeasure].style.prefix : "") + x + (HUCMeta[metric].style.units ? HUCMeta[activeMeasure].style.units : "");
    }
    else {
        return "N/A";
    }
...
function calcAverage(measure) {
    if (!HUCMeta[activeMeasure].style.avg) {
        var theSum = 0;
        var theCount = 0;
        ...
        HUCMeta[measure].style.avg = Math.round(theSum / theCount);
    }
}
function calcNHAverage(measure) {
    if (!NHmeta[activeNHMeasure].style.avg) {
        var theSum = 0;
        var theCount = 0;
        $.each(nhJSONData.features, function() {
            theSum = theSum + this.properties[measure];
            if (this.properties[measure] !== null) theCount++;
        });
        NHmeta[measure].style.avg = Math.round(theSum / theCount);
    }
}
